...
export function sendToFrontend(ws, type, data) {
    if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type, ...(typeof data === 'string' ? { content: data } : data) }));
    }
}
...
export function sendToFrontend(ws, role, content) {
...
const MESSAGE_HANDLERS = {
    ...
    'settings': async (data, ws) => {
        await handleSettings(data, ws);
        if (data.apiKey) {
            const prompt = "*[SYSTEM EVENT] Noa, sensei is looking for you, are you there?*";
    ...
    'get_core_memory': async (data, ws) => {
        ...
        const content = fs.existsSync(memFile) ? fs.readFileSync(memFile, 'utf8') : '';
    ...
    'save_core_memory': async (data, ws) => {
        ...
        console.log(pc.greenBright('[ System ] Core memory updated from frontend.'));
    },
    'input': async (data, ws) => {
        state.updateLastInteraction();
        console.log(pc.whiteBright(`\n[ Sensei ] ${data.content}`));
        await handleUserInput(data.content, ws);
    },
...
async function handleSettings(data, ws) {
    ...
    if (data.apiKey) {
        fs.writeFileSync(path.join(noaDir, '.env'), `OPENROUTER_API_KEY="${data.apiKey}"`);
    ...
    if (data.proactiveMode !== undefined) currentSettings.proactiveMode = data.proactiveMode;
    if (data.proactiveInterval !== undefined) currentSettings.proactiveInterval = data.proactiveInterval;
    if (data.models && Array.isArray(data.models) && data.models.length > 0) currentSettings.models = data.models;
    await saveSettings(currentSettings);
    
    if (data.storageDir && path.resolve(data.storageDir).toLowerCase() !== path.resolve(noaDir).toLowerCase()) {
        console.log(pc.yellowBright(`\n[ System ] Storage directory change detected. Migrating to ${data.storageDir}...`));
        try {
            if (!fs.existsSync(data.storageDir)) fs.mkdirSync(data.storageDir, { recursive: true });
            
            const filesToMove = ['memory.json', 'long_term_memory.txt', 'settings.json', '.env', 'todos.json', 'vector_memory.json'];
            ...
            if (fs.existsSync(path.join(noaDir, 'models'))) {
                fs.cpSync(path.join(noaDir, 'models'), path.join(data.storageDir, 'models'), { recursive: true });
            }
            if (fs.existsSync(path.join(process.cwd(), 'diary'))) {
                fs.cpSync(path.join(process.cwd(), 'diary'), path.join(data.storageDir, 'diary'), { recursive: true });
            }
            
            setDataDir(data.storageDir);
            console.log(pc.greenBright('[ System ] Migration complete. Restarting backend...'));
            
            ws.send(JSON.stringify({ type: 'message', role: 'system', content: 'Storage directory changed successfully. Please restart the application for changes to take effect.' }));
            
            setTimeout(() => process.exit(0), 1000);
            return;
        } catch (e) {
            console.error(pc.redBright(`[ System ] Migration failed: ${e.message}`));
            ws.send(JSON.stringify({ type: 'message', role: 'system', content: `Storage migration failed: ${e.message}` }));
        }
    }
...
export function startServer() {
    ...
    if (!expectedToken) {
        ...
        console.warn(pc.yellow("[ System ] WARNING: NOA_AUTH_TOKEN is not set (running outside Tauri?)."));
        console.warn(pc.yellow(`[ System ] Generated temporary dev token: ${expectedToken}`));
    }
    ...
    const app = new Elysia()
        ...
        .get('/audio/*', ({ params, set }) => {
            ...
            const audioFile = path.join(getAudioDir(), params['*']);
            if (fs.existsSync(audioFile)) {
                return Bun.file(audioFile);
            }
        ...
        .ws('/ws', {
            open(ws) {
                console.log(pc.cyanBright('[ System ] Desktop Frontend connected via WebSocket.'));
                state.setWs(ws);
            },
            async message(ws, message) {
                try {
                    ...
                    if (handler) {
                        ...
                        console.warn(pc.yellow(`[ System ] Unknown message type: ${data.type}`));
                    }
                } catch (err) {
            ...
            close(ws) {
                console.log(pc.yellowBright('[ System ] Desktop Frontend disconnected.'));
                state.setWs(null);
                setTimeout(() => {
                    if (!state.getWs()) {
                        console.log(pc.redBright('[ System ] No active connections. Shutting down orphaned backend...'));
                        process.exit(0);
                    }
                }, 5000);
            }
    ...
    console.log(pc.magentaBright(`[ System ] Elysia Backend listening on port ${app.server.port}`));
    
    setTimeout(() => {
        if (!state.getWs()) {
            console.log(pc.redBright('[ System ] No frontend connection after 60s. Shutting down...'));
            process.exit(0);
        }
    }, 60000);
}
